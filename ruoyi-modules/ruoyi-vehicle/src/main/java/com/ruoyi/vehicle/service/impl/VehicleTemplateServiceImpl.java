...
import com.ruoyi.vehicle.domain.AttributeTreeNode;
...
import com.ruoyi.vehicle.mapper.VehicleTemplateAttributeMapper;
...
public class VehicleTemplateServiceImpl implements IVehicleTemplateService {
    ...
    @Autowired
    private VehicleTemplateAttributeMapper attributeMapper;
    ...
    @Override
    public List<VehicleTemplateVo> selectTemplateList(VehicleTemplate query) {
        // 1. 查询模板列表（XML中过滤 deleted=0）
        List<VehicleTemplate> templateList = templateMapper.selectTemplateList(query);
        ...
        // 3. 批量查询属性路径（避免N+1）
        List<VehicleTemplateAttribute> allAttrList = attributeMapper.selectByTemplateIds(templateIds);

        // 4. 按模板ID分组
        Map<Long, List<VehicleTemplateAttribute>> attrGroupMap = allAttrList.stream()
                .collect(Collectors.groupingBy(VehicleTemplateAttribute::getTemplateId));
        ...
        return templateList.stream().map(template -> {
            VehicleTemplateVo vo = new VehicleTemplateVo();
            ...
            List<VehicleTemplateAttribute> attrList = attrGroupMap
    ...
    @Override
    public VehicleTemplateVo selectTemplateDetail(Long templateId) {
        // 1. 查询模板（XML中过滤 deleted=0）
        VehicleTemplate template = templateMapper.selectById(templateId);
        if (template == null) {
            throw new ServiceException("模板不存在");
        }

        // 2. 查询该模板的属性路径
        List<VehicleTemplateAttribute> attrList = attributeMapper.selectByTemplateId(templateId);
    ...
    public int insertTemplate(VehicleTemplate template) {
        // 1. 校验模板编码唯一性（仅未删除的记录）
        VehicleTemplate example = new VehicleTemplate();
        example.setTemplateCode(template.getTemplateCode());
        List<VehicleTemplate> existList = templateMapper.selectTemplateList(example);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int updateTemplate(VehicleTemplate template) {
        // 1. 校验模板是否存在
        VehicleTemplate dbTemplate = templateMapper.selectById(template.getTemplateId());
        if (dbTemplate == null) {
            throw new ServiceException("模板不存在");
        }
    ...
    public int deleteTemplates(List<Long> templateIds) {
        ...
        List<VehicleTemplate> existList = templateMapper.selectTemplateList(query);
     ...
     */
    private List<AttributeTreeNode> buildAttributeTree(List<VehicleTemplateAttribute> attrList, Map<Long, SysDictData> attrDictMap) {
        ...
        for (VehicleTemplateAttribute attr : attrList) {
    ...
    private void saveTreeNodes(Long templateId, List<AttributeTreeNode> nodes,
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            VehicleTemplateAttribute attr = new VehicleTemplateAttribute();
